    ...
    private addTasksForVariant(final Object variant, final boolean isTestVariant) {
        ...
        sourceSetNames.each { sourceSetName ->
            ...
            if (sourceSetName == 'androidTest') {
                extractIncludeProtosTask = TaskGenerator.maybeAddExtractIncludeProtosTask(project, sourceSetName, project.android.sourceSets['main'].proto, project.configurations['compile'])
            } else {
                extractIncludeProtosTask = TaskGenerator.maybeAddExtractIncludeProtosTask(project, sourceSetName)
            }
