...
class ProtobufJavaPlugin implements Plugin<Project> {
  ...
  */
  private addTasksForSourceSet(final SourceSet sourceSet) {
    def generateProtoTask =
        TaskGenerator.addGenerateProtoTask(project, sourceSet.name, [sourceSet])
    ...
    def extractIncludeProtosTask =
        TaskGenerator.maybeAddExtractIncludeProtosTask(project, sourceSet.name,
